...
if st.session_state[PAGE_KEY] == PK_LANDING:
    ...
    with col1:
        ...
        with st.form(key=FK_FILTERS, border=True):
            ...
            source_name_filters = st.multiselect(
                label="Source Name", options=source_name_extended, key=VK_NAME
            )
    ...
    with col2:
        ...
        if source_name_filters:
            source_name_filter = data["financial_commitment__source_name_extended"].isin(source_name_filters)
        else:
            source_name_filter = True

        lean_df = data.loc[year_filter & source_name_filter, lean_cols]
        ...
        chart.yaxis[0].ticker = AdaptiveTicker(desired_num_ticks=10, num_minor_ticks=4)
